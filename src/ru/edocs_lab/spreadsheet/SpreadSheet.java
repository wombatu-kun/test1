...
public final class SpreadSheet {
	...
	public static String[][] solve(int rowCount, int colCount, String lines[]) {
		if (0<rowCount && rowCount<=MAX_ROWS && 0<colCount && colCount<=MAX_COLS && lines!=null) {
			...
			ss.initCells(lines); //в многопоточном режиме вычисл€ем всЄ, что без ссылок
			ss.evaluateCells(); //рекурсивно дорешиваем оставшиес€ €чейки со ссылками
			return ss.toArray();
		} else {
			return null;
		}		
	...
	private void initCells(String lines[]) {		
		...
		//добиваем недостающие строки €чейками с NULL'€ми
	...
	private void parseInputLine(int row, String line) {
		...
		//добиваем недостающие €чейки NULL'€ми
	...
	private void createCell(int row, int col, String inStr) {
		...
		case '\'':
			...
			break;
		case '=': //€чейка с голой ссылкой обрабатываетс€ отдельно, потому что 
			//результат вычислени€ может быть как числом, так и текстом или NULLЄм,
			//а ссылка на текст или NULL в данном случае не €вл€етс€ ошибкой.
			if (inStr.toUpperCase().matches("^=[A-Z][1-9][0-9]*$")) {
				mSS[row][col] = new LinkCell(label, inStr);
				break;
			}
			//дл€ прочих выражений с= проходим в default
		default: //числа без=
			...
			if (!isProcessed()) {
				if(getPassedCells().add(getLabel())) {
					...
					if (link != null) {
						...
						setProcessed(true);
					} else {//нет такой €чейки
						setError(ErrMsg.OUTOFRNG);
					}
				} else {//€чейка не добавилась => уже проходили => зациклились
	...
	private class NumberCell extends Cell {		
		...
		private void doCellExpressionResult(Pattern ptOper) {			
			try {
				...
				if (val != null) {
					...
					//в любом случае, €чейка вычислена
				}
			} catch (NumberFormatException e) {
				setError(ErrMsg.GARBAGE);
		...
		private Long parseExpression(Pattern oper) 
			...
			try (Scanner scan = new Scanner(getInput())) {
				...
				while(part != null) {
					...
					if (operand == null) {//установили ошибку из €чейки по ссылке
		...
		private Long getCurrentOperandOrStop(String str) throws NumberFormatException {
			Long operand = null;
			if (str.charAt(0) > '9') {//начинаетс€ с буквы => операнд-ссылка
